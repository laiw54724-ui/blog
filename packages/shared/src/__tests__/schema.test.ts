...
describe('CreateEntrySchema', () => {
  ...
  it('rejects empty content', () => {
    expect(() => CreateEntrySchema.parse({ ...validEntry, content_markdown: '' })).toThrow();
  });

  it('rejects invalid entry_type', () => {
    expect(() => CreateEntrySchema.parse({ ...validEntry, entry_type: 'blog' })).toThrow();
  });

  it('rejects rating out of range', () => {
    expect(() => CreateEntrySchema.parse({ ...validEntry, rating: 0 })).toThrow();
    expect(() => CreateEntrySchema.parse({ ...validEntry, rating: 6 })).toThrow();
  });
