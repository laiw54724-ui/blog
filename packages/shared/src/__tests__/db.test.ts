...
import { createEntry, getEntries, getEntryById, getEntryBySlug, addTagsToEntry } from '../db'
...
describe('createEntry', () => {
  ...
  it('rejects invalid entry_type', async () => {
    const db = createMockDb()
    await expect(
      createEntry(db as any, { ...validEntry, entry_type: 'blog' })
    ).rejects.toThrow('Invalid entry_type: blog')
  })

  it('rejects invalid category', async () => {
    const db = createMockDb()
    await expect(
      createEntry(db as any, { ...validEntry, category: 'food' })
    ).rejects.toThrow('Invalid category: food')
  })

  it('rejects invalid status', async () => {
    const db = createMockDb()
    await expect(
      createEntry(db as any, { ...validEntry, status: 'deleted' })
    ).rejects.toThrow('Invalid status: deleted')
  })

  it('rejects invalid visibility', async () => {
    const db = createMockDb()
    await expect(
      createEntry(db as any, { ...validEntry, visibility: 'hidden' })
    ).rejects.toThrow('Invalid visibility: hidden')
  })
