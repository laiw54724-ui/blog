...
function makeDiscordRequest(env: any, payload: any, headers: Record<string, string> = {}) {
  return app.request(
  ...
  describe('APPLICATION_COMMAND (type 2)', () => {
    it('handles /post command with sync response', async () => {
      ...
      const payload = {
        ...
        data: {
          name: 'post',
          options: [{ name: 'content', value: '今天天氣真好', type: 3 }],
        },
        member: {
    ...
    it('handles /article command', async () => {
      ...
      const payload = {
        ...
        data: {
          name: '文章',
          options: [{ name: 'content', value: '深度分析文章內容', type: 3 }],
        },
        member: {
          user: { id: '123', username: 'testuser' },
        },
    ...
    it('handles /travel command', async () => {
      ...
      const payload = {
        ...
        data: {
          name: '書摘',
          options: [{ name: 'content', value: '讀完了原子習慣，很有啟發', type: 3 }],
        },
        member: {
          user: { id: '123', username: 'testuser' },
        },
    ...
    it('rejects empty content', async () => {
      ...
      const payload = {
        ...
        data: {
          name: 'post',
          options: [{ name: 'content', value: '', type: 3 }],
        },
        member: {
          user: { id: '123', username: 'testuser' },
        },
